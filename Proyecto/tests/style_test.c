...
int main (int argc, char const *argv[]) {
  ...
  sprintf(tag_sim, SIM);
  sprintf(tag_jefe, JEFE, "a");
  sprintf(tag_nave, NAVE, "a", "2");  
  

  sprintf(tag_sim_c, SIM_C);
  sprintf(tag_jefe_c, JEFE_C, "a");
  sprintf(tag_nave_c, NAVE_C, "a", "2");
  
  
  printf("\n[LEYENDA]\n");
  printf("%s %s", tag_sim_c,  "Esto es [SIM_C]\n");
  printf("%s %s", tag_jefe_c, "Esto es [JEFE_C] del equipo ""a""\n");
  printf("%s %s", tag_nave_c, "Esto es [NAVE_C] del equipo ""a"" con numero ""2""\n");
  printf(OK_SC      "          Esto es [OK_MSG_C]\n");
  printf(ERROR_SC   "          Esto es [ERROR_MSG_C]\n");

  printf("%s %s", tag_sim,  "Esto es [SIM]\n");
  printf("%s %s", tag_jefe, "Esto es [JEFE] del equipo ""a""\n");
  printf("%s %s", tag_nave, "Esto es [NAVE] del equipo ""a"" con numero ""2""\n");
  ...
  printf(JEFE_MC,         tag_jefe_c, OK_SC, "Esto es [JEFE_MC] del equipo ""a"", OK");
  printf(NAVE_MC,         tag_nave_c, OK_SC, "Esto es [NAVE_MC] del equipo ""a"" con numero ""2"", OK");
  printf(SIM_MC,          tag_sim_c, ERROR_SC, "Esto es [SIM_MC], ERR");
  printf(JEFE_MC,         tag_jefe_c, ERROR_SC, "Esto es [JEFE_MC] del equipo ""a"", ERR");
  printf(NAVE_MC,         tag_nave_c, ERROR_SC, "Esto es [NAVE_MC] del equipo ""a"" con numero ""2"", ERR");
  ...
  printf(JEFE_M,           tag_jefe, OK_S, "Esto es [JEFE_M] del equipo ""a"", OK");
  printf(NAVE_M,           tag_nave, OK_S, "Esto es [NAVE_M] del equipo ""a"" con numero ""2"", OK");
  printf(SIM_M,            tag_sim, ERROR_S, "Esto es [SIM_M], ERR");
  printf(JEFE_M,           tag_jefe, ERROR_S, "Esto es [JEFE_M] del equipo ""a"", ERR");
  printf(NAVE_M,           tag_nave, ERROR_S, "Esto es [NAVE_M] del equipo ""a"" con numero ""2"", ERR");
